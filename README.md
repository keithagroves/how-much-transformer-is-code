...
- Replacement-language frontiers: `dsl_frontier.py` (nested delete → positional → structural → content instruction sets), `posneg_curve.py` (full budget sweep), `mlp_rd.py` (lookup-table description length, including low-rank + frequent-token exceptions). Each writes a matching `*_results.csv` artifact.
- Concrete replacement example: `worked_example.py` emits `worked_example.json`, containing one real fitted attention-head formula and one real MLP token row decomposed into its rank-64 base and frequent-token correction.
